...
export default function ProcessesPage() {
  ...
  const [selected, setSelected] = useState('p-incoming');
  ...
  return <>
    <PageHeader eyebrow="Workflow Gateway · Mock Camunda" title="Центр процессов" description="Определения, версии, активные экземпляры и инциденты корпоративных процессов." actions={<PermissionGate permission="workflow.definition.edit"><button className="primary-button"><GitBranch size={16} /> Новая версия</button></PermissionGate>} />
    <div className="process-summary"><article><span className="tone-violet"><GitBranch size={18} /></span><div><strong>{processes.length}</strong><small>определения</small></div></article><article><span className="tone-teal"><Play size={18} /></span><div><strong>74</strong><small>активных экземпляра</small></div></article><article><span className="tone-gold"><History size={18} /></span><div><strong>7</strong><small>версий опубликовано</small></div></article><article><span className="tone-coral"><AlertOctagon size={18} /></span><div><strong>{processes.filter((p) => p.state === 'incident').length}</strong><small>инцидент</small></div></article></div>
