...
export default function ProcessesPage() {
  ...
  if (process.id === 'p-hr-leave' && leaves.data) {
    ...
    instances = [
      { id: 'inst-1', number: 'PR-2026-001', title: 'Департамент документооборота', step: 'Инициация', status: 'active', date: '2026-07-15' },
      { id: 'inst-2', number: 'PR-2026-002', title: 'Департамент инвестиций', step: 'Согласование', status: 'pending', date: '2026-07-16' },
    ];
  }

  return <>
    <PageHeader eyebrow={`${department.departmentCode} · Workflow Gateway`} title={department.isHrWorkspace ? 'Процессы' : 'Центр процессов'} actions={<PermissionGate permission="workflow.definition.edit"><button className="primary-button"><GitBranch size={16} /> Новая версия</button></PermissionGate>} />
