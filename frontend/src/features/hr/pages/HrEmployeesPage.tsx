...
export default function HrEmployeesPage() {
  ...
  return <>
    <PageHeader eyebrow="HR · Сотрудники" title="Сотрудники" actions={<><button className="secondary-button"><Download size={16} /> Экспорт</button><Link className="primary-button" to="/hr/hiring/add-employee"><UserPlus size={16} /> Добавить сотрудника</Link></>} />
    ...
    <div className="register-toolbar hr-toolbar"><label className="field-search"><Search size={16} /><input value={query} onChange={(event) => { const val = event.target.value; setQuery(val); setSearchParams(prev => { if (val) prev.set('query', val); else prev.delete('query'); return prev; }); }} placeholder="ФИО, должность или табельный номер" /></label><select value={department} onChange={(event) => setDepartment(event.target.value)}><option value="all">Все подразделения</option>{departments.map((item) => <option key={item}>{item}</option>)}</select><button className="toolbar-button"><SlidersHorizontal size={16} /> Колонки</button><span className="view-toggle"><button className="active"><List size={16} /></button><button><LayoutGrid size={16} /></button></span></div>
