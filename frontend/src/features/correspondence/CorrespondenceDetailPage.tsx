...
import { AlertCircle, ArrowLeft, CheckCircle2, Download, FileText, Forward, MessageSquare, MoreHorizontal, Paperclip, PenLine, Send, ShieldCheck, UserRound } from 'lucide-react';
...
export default function CorrespondenceDetailPage() {
  ...
  return <>
    ...
    <PageHeader eyebrow={`Входящая корреспонденция · ${item.number}`} title={item.subject} description={`${item.sender} · ${item.senderNumber} от ${formatDate(item.senderDate, locale)}`} actions={<><Link className="secondary-button" to="/correspondence/incoming"><ArrowLeft size={16} /> Реестр</Link><button className="secondary-button"><Download size={16} /> Скачать</button><PermissionGate permission="correspondence.register"><button className="primary-button" onClick={() => resolution.mutate()} disabled={item.status !== 'registered' || resolution.isPending}><Forward size={16} /> На резолюцию</button></PermissionGate><button className="icon-button bordered"><MoreHorizontal size={18} /></button></>} />
    ...
    <div className="case-layout">
      <div className="case-main">
        <div className="case-tabs"><button className="active">Обзор</button><button>Документ</button><button>Резолюция</button><button>Исполнение</button><button>Ответ</button><button>Вложения <b>{item.attachments.length}</b></button><button>История</button></div>
        ...
        <Section title="Вложения" meta={`${item.attachments.length} файла`}><div className="attachment-list">{item.attachments.map((file) => <button key={file.id}><span><FileText size={19} /></span><div><strong>{file.name}</strong><small>{file.size} · {file.kind === 'scan' ? 'Оригинал' : 'Приложение'}</small></div><Download size={16} /></button>)}</div></Section>
      </div>
      <aside className="case-aside">
        ...
        <Section title="Быстрые действия"><div className="quick-actions"><button><MessageSquare size={16} /> Добавить комментарий</button><button><Paperclip size={16} /> Добавить файл</button><button><PenLine size={16} /> Подготовить ответ</button><button><Send size={16} /> Запросить информацию</button></div></Section>
        <Section title="История действий"><div className="audit-list">{item.audit.map((event) => <div key={event.id}><i /><span><strong>{event.action}</strong><small>{event.actor} · {formatDate(event.at, locale, 'dd MMM · HH:mm')}</small><p>{event.detail}</p></span></div>)}</div><button className="panel-link plain"><ShieldCheck size={15} /> Полный журнал аудита</button></Section>
