export type ChartDatum = { label: string; value: number; color: string; detail?: string };
...
export function DonutChart({ data, centerValue, centerLabel, ariaLabel }: { data: ChartDatum[]; centerValue: string; centerLabel: string; ariaLabel: string }) {
  ...
  return <div className="dashboard-donut-layout">
    ...
    <div className="dashboard-chart-legend">{data.map((item) => <div key={item.label}><i style={{ background: item.color }} /><span><strong>{item.label}</strong>{item.detail && <small>{item.detail}</small>}</span><b>{item.value}</b></div>)}</div>
  </div>;
}

export function BarChart({ data, ariaLabel }: { data: ChartDatum[]; ariaLabel: string }) {
  const max = Math.max(1, ...data.map((item) => item.value));
  return <div className="dashboard-bar-chart" role="img" aria-label={ariaLabel}>
    <div className="dashboard-bar-plot">{data.map((item) => <div key={item.label} className="dashboard-bar-column"><div><span style={{ height: `${Math.max(item.value ? 12 : 2, item.value / max * 100)}%`, background: item.color }}><b>{item.value}</b></span></div><small>{item.label}</small></div>)}</div>
